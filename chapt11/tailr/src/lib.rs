...
// way without regex
fn normal_parse_num(val: &str) -> MyResult<TakeValue> {
    let signs: &[char] = &['+', '-'];
    let res = val
        .starts_with(signs)
        .then(|| val.parse())
...
#[cfg(test)]
mod tests {
    use super::{get_start_index, count_lines_bytes, parse_num, TakeValue::*};
    ...
    fn test_parse_num() {
        ...
        // Any noniteger string is invalid
    ...
    fn test_get_start_index() {
        ...
        // +0 ffrom a nonempty file returns an index that
        ...
        // When starting line/bytes is less than total lines/bytes,
...
fn my_print_lines(
    ...
    //println!("print lines num: {:?}, total: {:?}", &num_lines, &total_lines);
...
fn my_get_start_index(take_val: &TakeValue, total: i64) -> Option<u64> {
    ...
    match take_val {
        ...
        TakeNum(n) => {
            if n == &0i64 || total == 0i64 || n > &total {
                None
            } else if n > &0i64 && n <= &(total as i64) {
                Some((n-1).try_into().unwrap())
            } else if n < &0i64 && n <= &total {
                if n.abs() > (total as i64) {
                    Some(0)
                } else {
                    Some((n + total as i64).try_into().unwrap())
                }  
            } else {
                None
            }
...
// my
fn my_print_bytes<T: Read + Seek>(
    mut file: T,
    num_bytes: &TakeValue,
    total_bytes: i64,
) -> MyResult<()> {
    let start_byte_index = get_start_index(num_bytes, total_bytes);
    let mut num_lines = 0;
    let mut bytes = String::new();
    let mut buffer = Vec::new();
    if let Some(i) = start_byte_index {
