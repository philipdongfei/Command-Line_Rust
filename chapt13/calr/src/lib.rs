...
fn format_month(
    ...
    let mut days: Vec<String> = (1..first.unwrap().weekday().number_from_sunday())
            .into_iter()
    ...
    days.extend((first.unwrap().day()..=last.day()).into_iter().map(|num| {
...
#[cfg(test)]
mod tests {
    use super::{last_day_in_month, format_month, parse_int, parse_month, parse_year};
